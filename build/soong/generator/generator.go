...
func init() {
	android.RegisterModuleType("blackiron_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = blackironExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = blackironExpandVariables(ctx, dep_file)
	...
	cmd := blackironExpandVariables(ctx, String(g.properties.Cmd))
